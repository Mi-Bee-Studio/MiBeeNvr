...
function mockHeadResponse(opts: {
  ok: boolean;
  status?: number;
  ...
  return {
    ...
    headers: {
      get: (name: string) =>
        name.toLowerCase() === 'x-timelapse-codec' ? opts.codecHeader ?? null : null,
    },
...
describe('probeMergedRecordingCodec', () => {
  ...
  it('returns "h264" when the backend reports the H.264 codec', async () => {
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      mockHeadResponse({ ok: true, codecHeader: 'h264' }),
    );
  ...
  it('returns "h265" when the backend reports the H.265 codec', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      mockHeadResponse({ ok: true, codecHeader: 'h265' }),
    );
  ...
  it('returns "mjpeg" for mjpa outputs (JPEG frame cycler path)', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      mockHeadResponse({ ok: true, codecHeader: 'mjpeg' }),
    );
  ...
  it('returns null when the merged file is absent (404)', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      mockHeadResponse({ ok: false, status: 404 }),
    );
  ...
  it('returns null when the backend did not set the header', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      mockHeadResponse({ ok: true, codecHeader: null }),
    );
