<script lang="ts">
  ...
  function statusLabel(status: string): string {
    switch (status) {
      case 'healthy': return t('health.statusHealthy');
      case 'warning': return t('health.statusWarning');
      case 'error': return t('health.statusError');
      default: return t('health.statusUnknown');
    }
  }
  ...
  let prevFilter = cameraFilter;
  $effect(() => {
    if (cameraFilter !== prevFilter) {
      prevFilter = cameraFilter;
      page = 0;
    }
  });
...
<div class="min-h-screen th-bg-primary pt-[68px]">
  <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="mb-6">
      <h2 class="text-2xl font-bold th-text-primary mb-4">{t('health.events')}</h2>
