<script lang="ts">
 ...
 import { getTranscodingTasks, retryTranscodeTask } from '$lib/api/transcoding';
 ...
 import { AlertCircle, Video, RefreshCw } from 'lucide-svelte';
 ...
 // Retry failed task
 async function handleRetry(task: TranscodeTask) {
 	if (retryingId !== null) return;
 	retryingId = task.id;
 	try {
 		await retryTranscodeTask(task.id);
 		showToast(t('transcoding.retry_success', { id: String(task.id) }), 'success');
 		await loadTasks();
 	} catch {
 		showToast(t('transcoding.retry_failed'), 'error');
 	} finally {
 		retryingId = null;
 	}
 }
 				...
 				<table class="table">
 					<thead>
 						<tr>
 							...
 							<th class="min-w-[80px]">{t('transcoding.history.actions')}</th>
 						</tr>
 					</thead>
 					<tbody>
 							...
 							>
 								...
 								<td>
 									{#if task.status === 'failed'}
 										...
 										>
 											<RefreshCw size={14} class={retryingId === task.id ? 'animate-spin' : ''} />
 											{retryingId === task.id ? t('common.loading') : t('transcoding.retry')}
 										</button>
 									{/if}
 								</td>
 							</tr>
