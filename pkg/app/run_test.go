...
func TestRunFree_ServiceOrder(t *testing.T) {
	...
	expected := []string{"storage-migrator", "db", "startup-bg", "camera", "health", "merge", "rolling-merge", "motion-score", "mergeScheduler", "cleanup", "archive-deleter", "ws", "hls", "api-handler", "pprof-loopback"}
...
func TestRunFree_ServiceOrder_GB28181Enabled(t *testing.T) {
	...
	expected := []string{"storage-migrator", "db", "startup-bg", "camera", "health", "merge", "rolling-merge", "motion-score", "mergeScheduler", "cleanup", "archive-deleter", "srt", "gb28181", "ws", "hls", "api-handler", "pprof-loopback"}
	if len(svcs) != len(expected) {
		t.Errorf("Services() count = %d, want %d", len(svcs), len(expected))
	}
	for i, want := range expected {
...
func TestRunFree_ServiceOrder_DiscoveryEnabled(t *testing.T) {
	...
	expected := []string{"storage-migrator", "db", "startup-bg", "camera", "health", "merge", "rolling-merge", "motion-score", "mergeScheduler", "cleanup", "archive-deleter", "discovery", "mdns", "ws", "hls", "api-handler", "pprof-loopback"}
	if len(svcs) != len(expected) {
		t.Errorf("Services() count = %d, want %d", len(svcs), len(expected))
	}
	for i, want := range expected {
