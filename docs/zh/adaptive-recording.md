...
| 相机侧移动侦测 | `motion_source: camera:onvif` 时订阅相机的 ONVIF MotionAlarm 事件（`reason=onvif_motion`）——见 [ONVIF 指南](onvif-guide.md) |
...
MQTT 集成（[MQTT 集成](mqtt-integration.md)）的触发式录制同样走这条路径。
...
- [MQTT 集成](mqtt-integration.md) — 外部事件触发录制
