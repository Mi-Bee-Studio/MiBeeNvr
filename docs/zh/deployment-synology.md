...
> 不会 SSH？走下面的 GUI compose 路径即可。国内用户把 compose 里的 `image:` 换成阿里云地址也能加速：`registry.cn-hangzhou.aliyuncs.com/mickeybeehome/mibee-nvr:latest`
...
见[自动升级指南](deployment-autoupdate.md)。简言之：在项目目录 `docker compose pull && up -d`，或固定 tag（`mibeenvr:0.13.0`）以便可复现回滚。`/volume1/docker/mibee-nvr/data` 下的数据不受重建影响。
