...
**症状**: 仪表板摄像头网格或实时预览页面一直显示"缓冲中"或"加载中..."。视频从未开始播放。

**根本原因**: HLS.js 1.6+ 默认使用 `fetch` API 而不是 XHR。如果未将身份验证头注入到 fetch 请求中，服务器将返回 401 未授权，流无法加载。

**解决方案**:
1. 确保您运行的是 MiBee NVR v0.6.0 或更高版本，包含 `fetchSetup` 身份验证修复
2. 检查浏览器控制台（F12）查看 `.m3u8` 或 `.ts` 请求的 401 错误
3. 在尝试实时预览之前，验证摄像头正在录制（状态 = "录制中"）
4. 对于"重新连接"状态的摄像头，等待摄像头重新连接 — HLS 需要活动的录制流

### 流显示"SPS/PPS 不可用"错误 (503)

**症状**: HLS 端点返回 HTTP 503，消息为"SPS/PPS not available yet"

**解决方案**:
1. 摄像头开始录制后的前几秒是正常的 — 视频编码器需要生成关键帧数据
2. 前端会自动以指数退避重试（5s、10s、20s、40s）
3. 如果错误持续超过 60 秒，请检查：
   - 摄像头实际正在传输视频（使用 `ffprobe` 测试）
   - 录制处于活动状态（通过 API 检查摄像头状态）
   ```bash
...
**症状**: 单个摄像头实时预览正常，但仪表板网格显示所有摄像头卡在"缓冲中"

**解决方案**:
1. 仪表板同时加载多个流 — 检查 `hls.max_streams` 设置（默认：4）
2. 减少仪表板摄像头数量（最多 4 个，在 RPi 上越少越好）
3. 使用子流 URL 减少仪表板带宽：
   ```yaml
   cameras:
     - id: "cam1"
       sub_stream_url: "rtsp://192.168.1.100:554/sub"  # 较低分辨率流
       hls_max_fps: 10  # 限制仪表板帧率
   ```
4. 在低功耗设备（RPi 3B）上，限制仪表板最多 2 个 HLS 流

### 达到最大并发流数

**症状**: 流启动失败，错误为"maximum concurrent HLS streams reached"

**解决方案**:
1. 关闭未使用的仪表板或实时预览标签页
2. 如果您的硬件能够处理，增加 `max_streams`：
   ```yaml
   hls:
     max_streams: 6  # 从默认 4 增加到 6
   ```
3. 对某些摄像头使用快照缩略图而不是实时流：
   ```yaml
   cameras:
     - id: "cam-low-priority"
