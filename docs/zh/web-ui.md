...
- **AI 叠加**：开启[浏览器端 AI 检测](ai-detection-tuning.md)后，检测框直接画在画面上
- **点击画面**进入该摄像头的直播页

> H.265 摄像头在浏览器里通过 WASM 解码播放，无需 HTTPS 或插件；协议选择见[播放协议选择](streaming-protocol-selection.md)。
...
- 导航上的「有新版本可用」徽标提示可升级（见[升级指南](upgrade-guide.md)）
