...
MiBee NVR 存储层使用 SQLite 数据库，采用 `modernc.org/sqlite` 纯 Go 驱动，运行于 WAL 模式。生产环境（Banana Pi M5，`192.168.63.30`）的数据库包含 **67,000+ 条录制记录**，数据库文件 **86 MB**，每天新增录制数据约 **90 GB**（文件数据，非数据库大小——数据库仅存储元数据）。
