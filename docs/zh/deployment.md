...
1. **Host 网络模式**（推荐用于发现功能）：在 `docker-compose.yml` 中取消 `network_mode: host` 的注释，并删除 `ports` 段。容器将共享宿主机网络栈，从而支持组播。若宿主机的 9090 端口已被占用（如群晖 DSM），请在 `mibee-nvr.yaml` 中设置 `server.listen: ":9191"`（或其他空闲端口）——容器会直接绑定该宿主端口。健康检查（`mibee-nvr health`）会自动读取配置中的监听端口，无需额外设置。
