...
```bash
docker pull ghcr.io/mi-bee-studio/mibeenvr:0.13.0
docker run -d --name mibee-nvr ... ghcr.io/mi-bee-studio/mibeenvr:0.13.0
```
...
MiBee NVR 提供三种方式配置监听端口，按推荐顺序：

1. **Web UI（首选）**：设置 → 通用 → "Web 界面端口"。修改后保存即生效（服务自动重启）。这是装完之后改端口的首选入口，无需改配置文件或重启容器。
2. **安装时指定**：裸机 `./install.sh --port 9091`（首次安装时设置）。
3. **环境变量（NAS host 网络/Docker）**：`NVR_LISTEN_PORT=9091`（用于 `network_mode: host` 的 NAS 部署，Docker 端口映射在 host 模式下无效）。

> 旧的 `mibee-nvr.yaml` 中 `server.listen: ":9191"` 仍然有效，但上述三种方式更直观，无需手改配置文件。
...
1. **Host 网络模式**（推荐用于发现功能）：在 `docker-compose.yml` 中取消 `network_mode: host` 的注释，并删除 `ports` 段。容器将共享宿主机网络栈，从而支持组播。若宿主机的 9090 端口已被占用（如群晖 DSM），改用其他端口：装完后在 **Web UI → 设置 → 通用 → "Web 界面端口"** 修改（首选），或部署前设 `NVR_LISTEN_PORT=9191` 环境变量——容器会直接绑定该宿主端口。健康检查（`mibee-nvr health`）会自动读取配置中的监听端口，无需额外设置。
...
## NAS / 大内存设备优化（tmpfs 缓冲）

在内存充裕的主机（NAS、x86 小主机，8 GB+ RAM）上，频繁的小分片写入会让机械硬盘无法休眠，也加速 SD/eMMC 磨损。**零代码改动**的缓解方案是用 Linux `tmpfs` 作为 NVR 的临时录像目录，让小分片先写进内存，再由后台合并任务把大文件一次性落盘到机械硬盘。

```bash
# 1. 挂一块 tmpfs（按可用内存调整 size，建议不超过总内存的 25-30%）
sudo mount -t tmpfs -o size=2G tmpfs /mnt/nvr-tmp

# 2. 在 mibee-nvr.yaml 中把 storage.root_dir 指向 tmpfs 挂载点
#    storage:
#      root_dir: /mnt/nvr-tmp
#
# 然后把最终归档目录（机械硬盘）通过定时任务或合并输出指向持久盘。
```

注意事项：

- **断电会丢失 tmpfs 中的数据**。tmpfs 只适合做"滚动分片缓冲"，合并后的大文件必须及时 `mv` 到持久存储。中国的市电供应通常很稳定，但若不放心，可加一个小型 UPS。
- **size 要量力而行**。tmpfs 占用的是真实内存，过大会挤压 NVR 进程、HLS 直播和浏览器渲染的可用内存。2 GB 对多数 8 GB 主机是合理的起点。
- **v0.9.0 起的分段时长自适应**已经部分缓解了写盘频率：低内存设备（≤2 GB）用 30s 分段，高内存主机（>2 GB）自动用最长 2m 分段，直接把分片数减半。tmpfs 是在此基础上进一步消除碎碎念式 I/O。
