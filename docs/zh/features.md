...
> 适用于 MiBeeNvr v0.12.0（2026-08-31 发布）
...
| RTSP 输出 | 实时 | 第三方平台（群晖 Surveillance Station 等）直接取流，每机一址 `rtsp://<NVR-IP>:8554/<camera_id>` |
...
- **[自适应录制](adaptive-recording.md)**：画面安静时自动降为延时摄影级稀疏写入，检测到活动 / 异常声音 / 外部触发立即恢复全帧率——静态场景磁盘占用直降 75%–98%，事件第一帧不丢
...
- **下级级联**：NVR 作为下级平台注册到上级，转发目录/报警，上级可点播 NVR 本地录像；可按相机收敛上报目录、可选上报子码流（上行带宽减负）
