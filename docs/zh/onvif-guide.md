...
开启后（**设置 → 功能 → 自动发现摄像头**，或 YAML `auto_discover.enabled: true`），NVR 在后台自动发现接入局域网的 ONVIF 摄像头并自动入库，无需手动扫描——对标海康 NVR 的即插即用。

- **无鉴权设备**（如 ESP32 MiBeeCam）：直接启用并录像。
- **需鉴权设备**：若配置了默认凭据且有效则直接启用；否则标记为"待激活"，在摄像头页面点"激活"补充凭据后开始录像。
- 摄像头页面会弹出 toast 提示新发现的设备。

> 默认关闭。完整配置项见 [配置文档 - 自动发现配置](./configuration.md#自动发现配置)。

### 手动发现

以下方法需要用户主动触发（点击"扫描设备"），适合自动发现关闭或排查问题时使用。
