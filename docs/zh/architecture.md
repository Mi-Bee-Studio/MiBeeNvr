...
- **实时预览三种编码均已支持解码**（#131）：G.711 经 ITU-T 查找表；AAC 经 WebCodecs `AudioDecoder`（需 HTTPS 或 localhost——明文 HTTP 局域网下 AAC 实时预览音频降级并给出提示）；Opus 经 WebCodecs `OpusDecoder`。见 `web/src/lib/audio-player.ts` 与 `web/src/lib/decoders/`。G.711 不受影响，录像中的 AAC 音频不受影响。（早期仅支持 G.711，AAC/Opus 实时预览在 #131 加入。）
...
- **AAC/Opus 实时预览解码（#131 已支持）**：实时预览现支持全部三种编码——G.711 经查找表（不受影响）；AAC 经 WebCodecs `AudioDecoder`（需 HTTPS 或 localhost，明文 HTTP 局域网下降级并提示）；Opus 经 WebCodecs `OpusDecoder`（见 `web/src/lib/decoders/`）。录像中的 AAC 音频不受影响。录制回放使用浏览器原生 MP4 解码器。
