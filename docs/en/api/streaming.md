...
Segment and partial-segment file names are generated by the server — there is
nothing to construct by hand. The playlist references them with relative URLs;
hand the playlist to any HLS player (ffplay, VLC, hls.js, AVPlayer) and it
fetches them automatically.

**Response:** HLS playlist or segment file content

**Notes:**
- The playlist file name is `index.m3u8` (multivariant playlist);
  `stream.m3u8` and `playlist.m3u8` are accepted as compatibility aliases.
  Any other unregistered file name returns `404 unknown HLS resource`.
- A playlist request blocks until the first segment is produced (server-side
  cap of 30 s). A freshly added camera may answer
  `503 SPS/PPS not available yet` — retry once the first keyframe has arrived.
- Low-Latency HLS is on by default (`hls.low_latency`, see the configuration
  docs): playlists carry `#EXT-X-PART` partial segments, and LL-HLS clients
  may poll with `_HLS_msn`/`_HLS_part`. Plain players consume whole segments
  from the same playlist. Set `hls.low_latency: false` and restart to serve
  classic segment-only playlists.
