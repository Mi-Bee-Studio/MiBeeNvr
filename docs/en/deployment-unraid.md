...
- **Manual / pinned (recommended for recording-critical setups):** set the image tag to a specific release (e.g. `mibeenvr:0.12.0`), then `docker pull` + recreate to update. Pinning also gives you a clean rollback target.
