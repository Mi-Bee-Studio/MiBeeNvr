...
func (d *DB) EnqueueTask(ctx context.Context, task *TranscodeTask) error {
	q := `INSERT INTO transcoding_tasks (camera_id, recording_id, input_path, input_format, output_path, output_format, status, progress, created_at, original_deleted)
		VALUES (?, ?, ?, ?, ?, ?, 'pending', 0, ?, 0);`
	result, err := d.db.ExecContext(ctx, q, task.CameraID, task.RecordingID, task.InputPath, task.InputFormat, task.OutputPath, task.OutputFormat, task.CreatedAt)
	if err != nil {
		return err
	}
...
func (d *DB) DequeueTask(ctx context.Context) (*TranscodeTask, error) {
	...
	q := `UPDATE transcoding_tasks SET status = 'running', started_at = COALESCE(started_at, ?)
		...
		RETURNING id, camera_id, recording_id, input_path, input_format, output_path, output_format,
			status, progress, error, created_at, started_at, completed_at, original_deleted;`
	...
	err = row.Scan(
		...
		&task.CreatedAt, &task.StartedAt, &task.CompletedAt, &task.OriginalDeleted,
	)
	if err != nil {
		return nil, err
	}
...
func (d *DB) GetTasksByStatus(ctx context.Context, status string) ([]TranscodeTask, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&t.CreatedAt, &startedAt, &completedAt, &t.OriginalDeleted,
		); err != nil {
			return nil, err
		}
...
func (d *DB) GetTaskByID(ctx context.Context, id int64) (*TranscodeTask, error) {
	...
	err := row.Scan(
		...
		&task.InputPath, &task.InputFormat, &task.OutputPath, &task.OutputFormat,
		&task.Status, &task.Progress, &task.Error,
		&task.CreatedAt, &startedAt, &completedAt, &task.OriginalDeleted,
	)
	if err != nil {
