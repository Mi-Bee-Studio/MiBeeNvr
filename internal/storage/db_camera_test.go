...
// TestUpsertCamera_UnarchivesOnConflict confirms that re-adding a previously
// archived camera (GB28181 auto-enroll after device return, manual re-add)
// resurrects the row: without the archived reset, the camera would record but
// stay invisible to ListCameras (archived=0 filter).
func TestUpsertCamera_UnarchivesOnConflict(t *testing.T) {
	db := newCameraDB(t)
	ctx := context.Background()
	const camID = "cam-resurrect"
	require.NoError(t, db.UpsertCamera(ctx, camID, "Old Name", "gb28181", "", "", "", "", "", "", "", ""))
	require.NoError(t, db.ArchiveCameraDB(ctx, camID))

	// Archived: hidden from the active list.
	cams, err := db.ListCameras(ctx)
	require.NoError(t, err)
	require.NotContains(t, cameraIDs(cams), camID)

	// Re-add via the normal enrollment path — same ID, fresh row content.
	require.NoError(t, db.UpsertCamera(ctx, camID, "Re-added", "gb28181", "", "", "", "", "", "", "", ""))

	cams, err = db.ListCameras(ctx)
	require.NoError(t, err)
	require.Contains(t, cameraIDs(cams), camID, "re-added camera must be unarchived (visible in ListCameras)")
	for _, c := range cams {
		if c.ID == camID {
			require.False(t, c.Archived)
			require.Equal(t, "Re-added", c.Name)
		}
	}
}

func cameraIDs(cams []CameraRow) []string {
