...
func (cm *CameraManager) RediscoverAndReconnect(ctx context.Context, cameraID string) (found bool, err error) {
	...
	if rerr != nil {
		// Not found / unsupported — not a hard error. Log and bail.
	...
	// Keep cam.URL in sync with the endpoint (ONVIF cameras key off ONVIFEndpoint,
	// but some legacy code paths read URL).
