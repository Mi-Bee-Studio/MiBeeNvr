...
- **`handleCameraProtocols` codec resolution** (`handler.go`): the `encoding` field MUST reflect the real runtime codec, not the stale DB `encoding`/`stream_encoding` columns — otherwise the frontend misroutes H.265 cameras to MSE-based HLS/FLV (black screen on Chromium). Resolution order: (1) probe the running recorder via `getCodecParams(rec)` — authoritative (packet-derived), but returns `""` before the first packet AND while a xiaomi recorder is mid-P2P-reconnect (its codec field resets); (2) fall back to `hls.Manager.CodecFor(id)` — the HLS muxer freezes the codec at stream-start and keeps it across recorder reconnects, so this covers the reconnect window; (3) only then the stored DB value. Do NOT re-add a protocol gate that skips the recorder probe for non-ONVIF cameras (PR #101 did this to avoid a transient mislabel — that regression no longer applies because the recorder returns `""` until its codec-OK flag is set, and the HLS fallback now covers reconnects).
