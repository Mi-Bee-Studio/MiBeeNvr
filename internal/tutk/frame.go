...
// isADTS checks if the payload starts with an AAC ADTS syncword (0xFFF).
...
func parseAudioParams(payload []byte, fi *FrameInfo) (sampleRate uint32, channels uint8) {
	if isADTS(payload) && len(payload) >= 7 {
		// ADTS header: syncword(12) + ID(1) + layer(2) + protection_absent(1)
		// + profile(2) + sampling_frequency_index(4) + private_bit(1)
		// + channel_configuration(3)
		sampleRateIndex := (payload[2] >> 2) & 0x0F
		adtsSampleRates := []uint32{96000, 88200, 64000, 48000, 44100, 32000, 24000, 22050, 16000, 12000, 11025, 8000, 7350, 0, 0, 0}
		if int(sampleRateIndex) < len(adtsSampleRates) {
