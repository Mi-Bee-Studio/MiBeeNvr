...
- **Recording gate (`RecordEnabled`)**: `baseRecorder` reads `cfg.RecordEnabled` in `writeFrames()` (nil/true = record, false = live-only — stream stays alive, no disk writes). H264/H265/MJPEG/HTTPJPEG/ONVIF inherit this via `baseRecorder`. **IMPORTANT**: in `writeFrames()` the NAL-parse + parameter-set capture (SPS/PPS/VPS via `handleParamSet`) runs BEFORE the `RecordEnabled` gate, and the gate itself is just a `continue` (drain). This ordering is load-bearing: live preview (HLS/WebRTC/WS) reads the codec param sets via `getCodecParams()`, and if the gate drained frames before capturing SPS/VPS/PPS, every live-preview endpoint would return 503 "waiting for video stream" → permanent grey/black screen for cameras with `recording_enabled=false`. Do not move the gate above the param-set capture. Plugin-style recorders do NOT inherit this: `XiaomiRecorder` gates its own writes via `recordDisabled()` (see `internal/xiaomi/AGENTS.md`), and `IngestRecorder` (SRT/RTMP push-in) does NOT honor `RecordEnabled` yet (push-in streams rarely need live-only mode; file an issue if needed). When adding a new recorder that writes segments, either inherit `baseRecorder` or add a `RecordEnabled` gate to every disk-write path — otherwise `recording_enabled=false` silently keeps recording (issue #73).
