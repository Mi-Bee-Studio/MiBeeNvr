...
- **Audio is H264/H265-only (current limitation)**: `MJPEGRecorder` and `HTTPJPEGRecorder` are video-only — no audio track SETUP, no `BroadcastAudio`, no muxer. They store JPEG frames to a directory (NOT MP4). The ESP32 MiBeeCam firmware serves an RTSP stream with MJPEG video + G.711 audio, but recording its audio is blocked: (a) MJPEGRecorder would need the audio pipeline retrofitted, and (b) browsers cannot play MJPEG inside an MP4 container via `<video>`, so muxing MJPEG+audio for playback is not viable (a MJPEG-in-MP4 file is downloadable but not browser-playable). `getCodecParams` (`internal/api/handlers_stream.go`) MUST have a case for every recorder type so the live recorder probe returns a non-empty codec — MJPEG/HTTPJPEG return `EncJPEG`/`FormatMJPEG` so `/protocols` and the camera-list encoding backfill advertise the `mjpeg` player; missing a case here breaks live preview + Surveillance grid rendering for that camera type.
